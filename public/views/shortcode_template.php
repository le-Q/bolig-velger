...
*/
  ...
  (!empty($_GET['action']) && $_GET['action'] == 'elementor')
	...
	<div class="hotspots-image-container">
		<img width="<?php echo $settings['img_width']; ?>" height="<?php echo $settings[
  'img_height'
]; ?>" src="<?php echo $settings['img_url']; ?>" alt="<?php echo esc_attr(
  $settings['img_alt']
); ?>" class="hotspots-image skip-lazy" data-id="<?php echo $settings[
  'image_id'
]; ?>" data-no-lazy="1" data-lazy-src="" data-lazy="false">
	</div>
	<?php
  // There are hotspots! Show the interactive image
  ?>
<?php else: ?>
	...
	<style>
		...
		<?php foreach ($formatted_styles as $style): ?>#<?php echo $settings[
  'spot_id'
]; ?>.hotspot-<?php echo $style['name']; ?> {
		...
		#<?php echo $settings['spot_id']; ?>.hotspot-<?php echo $style[
  'name'
]; ?>:hover,
		#<?php echo $settings['spot_id']; ?>.hotspot-<?php echo $style[
  'name'
]; ?>.hotspot-active {
			fill: <?php echo esc_html(
     get_post_meta($hotspot['action'], '_cmb2_leilighet_status', true)
   ) == 'ledig'
     ? "#F00"
     : '#0F0'; ?>;
		...
		<?php endforeach; ?>#<?php echo $settings['spot_id']; ?>.leaflet-tooltip,
...
*/
  ?>
	...
	<div class="hotspots-container <?php echo $settings[
   'urls_class'
 ]; ?> layout-<?php echo $settings['layout']; ?> event-<?php echo $settings[
   'event_trigger'
 ]; ?>" id="<?php echo $settings[
  'spot_id'
]; ?>" data-layout="<?php echo $settings[
  'layout'
]; ?>" data-trigger="<?php echo $settings['event_trigger']; ?>">
		<!--<div class="hotspots-container">-->
		<div class="bolig-velger-area hotspots-container">
     ...
     require $this->get_plugin_dir() . '/public/views/more_info_template.php';
		...
		<map name="hotspots-image-<?php echo $settings[
    'image_id'
  ]; ?>" class="hotspots-map">
    ...
    $new_window = !empty($hotspot['action-url-open-in-window'])
      ? $hotspot['action-url-open-in-window']
      : '';
    $target_window = $new_window == 'on' ? '_new' : '';
    $target_url = !empty($hotspot['action-url-url'])
      ? $hotspot['action-url-url']
      : '';
    $area_class = $target == 'url' ? 'url-area' : 'more-info-area';
    $href =
      $target == 'url'
        ? $target_url
        : '#hotspot-' . $settings['spot_id'] . '-' . $key;
    ...
    if (
      empty($settings['img_settings']['_bv_has_multiple_styles']['0']) ||
      $settings['img_settings']['_bv_has_multiple_styles']['0'] != 'on' ||
      empty($hotspot['style'])
    ) {
      $color_scheme = '';
    } else {
    ...
    ?>
				<area shape="poly" coords="<?php echo $coords; ?>" href="<?php echo $href; ?>" title="<?php echo $title; ?>" alt="<?php echo esc_attr(
  $title
); ?>" data-action="<?php echo $target; ?>" data-color-scheme="<?php echo $color_scheme; ?>" target="<?php echo $target_window; ?>" class="<?php echo $area_class; ?>">
			<?php endforeach; ?>
		</map>

		<?php if (!empty($_GET['bv_debug'])): ?>
   ...
   ?>
			<div class="hotspot-info <?php echo $color_scheme_class; ?>" id="hotspot-<?php echo $settings[
  'spot_id'
]; ?>-<?php echo $key; ?>">
				<?php if (!empty($hotspot['action'])) {
      if ('bigcommerce' === $hotspot['action']) {
        echo DrawAttention_BigCommerce_Action::render_hotspot_content(
          $hotspot,
          $settings
        );
        echo '</div>';
      ...
      'boligvelger_hotspot_title',
      '<h2 class="bolig-velger-title"> Leilighet ' .
        get_the_title($hotspot['action']) .
        '</h2>',
      $hotspot
    ); ?>
        ...
        $hotspot['detail_image_id'],
        apply_filters(
          'bv_detail_image_size',
          'large',
          $hotspot,
          $settings['img_post'],
          $settings['img_settings']
				...
				<?php elseif (
      empty($hotspot['detail_image_id']) &&
      !empty($hotspot['detail_image'])
    ): ?>
					<div class="hotspot-thumb">
				...
				<div class="hotspot-content">
					...
					<div class="aprt-content">
						<?php $image = get_the_post_thumbnail($hotspot['action']); ?>
         ...
         echo apply_filters(
           ...
           do_shortcode(
             $wp_embed->autoembed(
               $wp_embed->run_shortcode($hotspot['description'])
             )
           )
         );
       } ?>
       ...
       $status = get_post_meta(
         $hotspot['action'],
         '_cmb2_leilighet_status',
         true
       );
       $nr = get_post_meta($hotspot['action'], '_cmb2_leilighet_nr', true);
       $floor = get_post_meta(
         $hotspot['action'],
         '_cmb2_leilighet_etasje',
         true
       );
       $area = get_post_meta(
         $hotspot['action'],
         '_cmb2_leilighet_bruttoareal',
         true
       );
       $rooms = get_post_meta(
         $hotspot['action'],
         '_cmb2_leilighet_antall',
         true
       );
