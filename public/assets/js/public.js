(function ($, hotspots, undefined) {
  ...
  var renderPoly = function (coords, map, img, areaData) {
    ...
    if (
      carName.filter((x) => x.id == areaData.action).map((x) => x.status) ==
      "Ledig"
    ) {
      test_result = "ledig";
    } else {
    ...
    for (var i = 0; i < tableRow.length; i++) {
      tableRow[i].onclick = function (e) {
        ...
        if ($(this).hasClass("hotspot-active")) {
          $(this).trigger("active.responsilight");
          //console.log(this);
        } else {
          $(this).trigger("inactive.responsilight");
          //console.log(this);
        }
        let oldActive = $(this).siblings(".hotspot-active");
        if (oldActive.length) {
          oldActive.removeClass("hotspot-active");
        }
      };
  ...
  var shapeEvents = function (shape, areaData) {
    ...
    shape.on(
      "touchstart touchmove touchend click mouseover mouseout",
      function (e) {
        switch (e.type) {
          case "touchstart":
            moved = false;
